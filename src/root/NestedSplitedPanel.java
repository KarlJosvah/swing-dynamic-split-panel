...
public class NestedSplitedPanel extends JPanel {
	...
	public NestedSplitedPanel(JPanel defaultPanel) {
		this.contentPanel = defaultPanel;
	...
	public void performSplit(int orientation, boolean newPanelFirst, double splitRatio) {
		...
		boolean isJFrameContentPane = (window instanceof javax.swing.JFrame && ((javax.swing.JFrame) window).getContentPane() == this);
		...
		NestedSplitedPanel newPanel = new NestedSplitedPanel(new DefaultPanel());
		...
		setupDivider(splitPane);
		replaceComponent(parent, this, splitPane, wasLeftInParentSplit, isJFrameContentPane, window);

		Container root = (window != null) ? window : parent;
		root.revalidate();
		root.repaint();
	...
	private static void replaceComponent(Container parent, Component oldComp, Component newComp, boolean wasLeftInParentSplit, boolean isJFrameContentPane, java.awt.Window window) {
		if (isJFrameContentPane && window instanceof javax.swing.JFrame && newComp instanceof Container) {
			((javax.swing.JFrame) window).setContentPane((Container) newComp);
			...
			parent.add(newComp, BorderLayout.CENTER);
		}
	}

	private static void setupDivider(JSplitPane splitPane) {
		...
		if (divider == null) {
			for (Component c : splitPane.getComponents()) {
				if (c instanceof BasicSplitPaneDivider || c.getClass().getName().contains("Divider")) {
					divider = c;
					break;
				}
			}
	...
	private static void showDividerMenu(MouseEvent e, JSplitPane splitPane) {
		JPopupMenu menu = new JPopupMenu();
		int orientation = splitPane.getOrientation();

		if (orientation == JSplitPane.HORIZONTAL_SPLIT) {
			JMenuItem joinLeft = new JMenuItem("Join Left");
			joinLeft.addActionListener(al -> joinSplit(splitPane, false));
			JMenuItem joinRight = new JMenuItem("Join Right");
			joinRight.addActionListener(al -> joinSplit(splitPane, true));

			menu.add(joinLeft);
			menu.add(joinRight);
		} else {
			JMenuItem joinUp = new JMenuItem("Join Up");
			joinUp.addActionListener(al -> joinSplit(splitPane, false));
			JMenuItem joinDown = new JMenuItem("Join Down");
	...
	private static void joinSplit(JSplitPane splitPane, boolean keepFirstChild) {
		Container parent = splitPane.getParent();
		if (parent == null) return;

		Component keptChild = keepFirstChild ? splitPane.getLeftComponent() : splitPane.getRightComponent();
		if (keptChild == null) return;

		java.awt.Window window = SwingUtilities.getWindowAncestor(splitPane);
		boolean isJFrameContentPane = (window instanceof javax.swing.JFrame && ((javax.swing.JFrame) window).getContentPane() == splitPane);
		...
		replaceComponent(parent, splitPane, keptChild, wasLeftInParentSplit, isJFrameContentPane, window);
